...
<body class="bg-[#F1F9E4] font-sans">
    ...
    function openModal(productId) {
        ...
        modalDetails.innerHTML = `
            ...
            <p class="text-gray-700">Barcode: ${barcode}</p>
    ...
    function getTotalCartItems() {
        return <?= json_encode(array_reduce($_SESSION['cart'] ?? [], fn($carry, $item) => $carry + $item['jumlah'], 0)) ?>;
    }
    ...
    function prepareQuantity(productId) {
        ...
        const existingTotal = getTotalCartItems();
        const newTotal = existingTotal + qty;
        
        if (newTotal > 10) {
            const canAdd = 10 - existingTotal;
            showAlert(`Maksimal total 10 produk di keranjang! Anda hanya bisa menambahkan ${canAdd} produk lagi.`);
            return false;
        }
    ...
    document.getElementById("scanButton").addEventListener("click", () => {
        const useCamera = confirm("Gunakan kamera (OK) atau scanner USB (Cancel)?");
        if (useCamera) {
    ...
    function openUsbScannerModal() {
        ...
        usbScannerStatus.innerHTML = `
            <i class="fas fa-circle-notch fa-spin text-[#61892F] text-2xl"></i>
            <p class="mt-2 text-gray-700">Menunggu input dari scanner USB...</p>
            <p class="text-sm text-gray-500">Modal akan tetap terbuka setelah scanning</p>
        `;
    ...
    function processScannedBarcode(barcode) {
        ...
        usbScannerStatus.innerHTML = `
            ...
            <p class="mt-2 text-gray-700">Barcode berhasil discan: ${barcode}</p>
            <p class="text-sm text-gray-500">Modal tetap terbuka untuk scan berikutnya</p>
        `;
        ...
        try {
            ...
            if (data.success && data.product_id) {
                ...
                if (product) {
                    ...
                    // Cek total produk di keranjang
                    const totalCartItems = getTotalCartItems();
                    if (totalCartItems >= 10) {
                        showAlert("Maksimal total 10 produk di keranjang!", "error");
                        return;
                    }
                    ...
                    if (stock > 0 && addButton && !addButton.disabled) {
                        const maxToAdd = 10 - totalCartItems;
                        const qtyToAdd = Math.min(1, maxToAdd);
                        
                        document.getElementById(`quantity-${data.product_id}`).textContent = qtyToAdd;
                        document.getElementById(`qty-${data.product_id}`).value = qtyToAdd;
                        
                        // Kirim form secara manual tanpa menutup modal
                        ...
                        fetch(form.action, {
                            ...
                            showAlert(`Produk "${data.product_name}" berhasil ditambahkan (${qtyToAdd} pcs)`, "success");
                        }).catch(error => {
                            console.error('Error:', error);
                            showAlert("Gagal menambahkan produk ke keranjang", "error");
                        });
                    }
